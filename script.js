...
contactForm?.addEventListener("submit", (event) => {
  ...
  if (window.location.protocol === "file:") {
    button.textContent = "Nelze lokalne";
    if (formNote) {
      formNote.textContent = "Odesilani funguje az po nasazeni na Vercel, ne pri otevreni souboru index.html.";
    }
    window.setTimeout(() => {
      button.textContent = originalText;
      button.disabled = false;
  ...
  fetch("/api/contact", {
    ...
    .then(async (response) => {
      if (!response.ok) {
        ...
        throw new Error(error.message || "Zpravu se nepodarilo odeslat.");
      }
